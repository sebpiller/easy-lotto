...
class LotoGridRecognizer {
    ...
    suspend fun extractAllNumbers(input: InputImage): List<LotoNum> {
        ...
        val recognized = recognize(InputImage.fromBitmap(processedBitmap, 0)) ?: return listOf<LotoNum>()
    ...
    private fun processFoundRawText(
        ...
        Log.d("LotoGridRecognizer", "Found raw text: '${element.text}' with enough confidence")
    ...
    private suspend fun recognize(image: InputImage): Text? = suspendCancellableCoroutine { cont ->
        recognizer
            ...
            .addOnFailureListener {
                Log.e("LotoGridRecognizer", "Text recognition failed", it)
                cont.resume(null)
            }
    }
}