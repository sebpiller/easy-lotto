package ch.sebpiller.easy.loto.ocr
...
internal class LotoGridRecognizerTest {

    @Test
    fun test_can_read_loto_grid_1() = runBlocking {
        val res = recognizeCells("loto_grid.webp")
        ...
        val grid = LotoGrid.fromNumbers(res)
        grid.print()
    ...
    @Test
    fun test_can_read_loto_grid_2() = runBlocking {
        val res = recognizeCells("loto_grid2.jpg")
        Assert.assertEquals(15, res.size)
    ...
    @Test
    fun test_can_read_loto_grid_3() = runBlocking {
        val res = recognizeCells("loto_grid3.jpg")
        Assert.assertEquals(15, res.size)
    ...
    @Test
    fun test_can_read_loto_grid_4() = runBlocking {
    ...
    @Test
    fun test_can_read_loto_from_real_photo() = runBlocking {
    ...
    @Test
    fun test_can_read_loto_from_real_photo_1() = runBlocking {
    ...
    private suspend fun recognizeCells(img: String): List<LotoNum> {
        val start = System.currentTimeMillis()
        val context = InstrumentationRegistry.getInstrumentation().context
        val imageStream = context.assets.open(img)

        val originalBitmap = BitmapFactory.decodeStream(imageStream)
        val coll = LotoGridRecognizer().extractAllNumbers(InputImage.fromBitmap(originalBitmap, 0))

        Log.i("LotoGridRecognizerTest", "Read duration ${System.currentTimeMillis() - start}ms")
